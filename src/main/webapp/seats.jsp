...
<body class="bg-gray-600" style="font-family: poppins;">
	...
	<div class="">
		<div class="grid grid-cols-1 lg:grid-cols-2">
			<div class="flex pb-3">	
				<h3 class="text-white text-xl">Seats Details</h3>
			...
			<table id="myTable">
				<thead  class=" bg-blue-600 pl-4 text-blue-100 w-[800px]  h-9 ">
					<tr class="text-center">
						<th class="pr-[110px] pl-5">ID</th>
						<th class="pr-[110px]">BUS NUMBER</th>
						<th class="pr-[110px]">BUS TYPE</th>
						<th class="pr-[110px]">RESERVED</th>
						<th class="pr-[110px]">RESERVING</th>
						<th class="pr-[110px]">DATE</th>
				...
				<tbody>
						<%
							...
							ResultSet rs =statement2.executeQuery("SELECT * FROM seats");
							while(rs.next()){
							%>
							<tr class="h-9 even:bg-gray-800 text-blue-100">
								...
								<td><%= rs.getString("reserving") %></td>
								<td>
									<%= formattedDateTime %>
								<td>
									<a href="seat_delete.jsp?id=<%=rs.getString("id") %>">									
		...
		<div class="h-sreen  w-screen flex items-center justify-center bg-white">
			<div id="form_container" class="m-auto top-[0%] absolute h-[480px] w-[350px] bg-gray-800 rounded-md hidden inset-0 ">
				...
				<form action="seats.jsp" method="post" class="pt-5 flex-col items-center justify-center pl-20 ">
					...
					<label class="text-white">Reserving</label><br>
					<input class="h-9 rounded-md outline-none bg-blue-50" type="text" name="reserving" required><br><br>
					
					<label class="text-white">Reserved</label><br>
					<input class="h-9 rounded-md outline-none bg-blue-50" type="text" name="reserved" required><br><br>
...
<%
	if(request.getParameter("submit") != null){
		try{
			...
			String Bus_Seats = request.getParameter("reserving");
			...
			int insert =statement.executeUpdate("INSERT INTO seats (bus_number,bus_type,reserving,reserved) VALUES ('"+Bus_Number+"','"+Bus_Type+"','"+Bus_Seats+"','"+Reserved+"')");
			if(insert>0){
				%>
	       		<script>
						alert("New Bus Added Successfully");
						window.location.href="seats.jsp";
					</script>
	       	<% 
			}else{
				%>
